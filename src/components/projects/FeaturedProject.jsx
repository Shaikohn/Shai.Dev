...
export default function FeaturedProject({ project, index, language, labels, onOpenGallery }) {
  ...
  const galleryImages = Array.isArray(project.images) ? project.images : project.images[language] ?? project.images.es;
  const openGallery = () => onOpenGallery({ ...project, images: galleryImages });
  return (
    <article className="group grid items-center gap-8 opacity-90 transition-all duration-300 hover:opacity-100 md:grid-cols-[1.05fr_0.95fr] md:gap-10">
      <div className={`${reverse ? "md:order-2" : ""} order-1 max-w-xl text-center transition-all duration-300 group-hover:-translate-y-1 md:text-left`}>
        ...
        {project.ownership && <p className="mb-4 text-sm text-zinc-300">{project.ownership[language]}</p>}
        ...
        <div className="flex flex-wrap justify-center gap-4 md:justify-start">
          ...
          <button type="button" onClick={openGallery} className="cursor-pointer rounded-full border border-white/10 px-5 py-2.5 text-sm font-medium text-zinc-400 transition-all hover:border-white/25 hover:bg-white/10 focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-sky-400">{labels.screenshots}</button>
        </div>
        {project.links.caseStudy && <div className="mt-3 flex justify-center md:justify-start"><Link to={project.links.caseStudy} className="text-sm font-medium text-sky-400 transition-colors hover:text-sky-300">{labels.caseStudy} →</Link></div>}
      </div>
      <div className={`${reverse ? "md:order-1" : "md:justify-end"} order-2 flex justify-center`}>
        <div className={`${project.type === "mobile" ? "rounded-[32px]" : "rounded-2xl"} relative border border-white/10 bg-black/20 p-2 shadow-[0_20px_80px_rgba(0,0,0,0.35)] transition-all duration-300 group-hover:scale-[1.02]`}>
          ...
          <button type="button" onClick={openGallery} className="cursor-pointer border-0 bg-transparent p-0" aria-label={`${labels.screenshots}: ${project.title}`}>
